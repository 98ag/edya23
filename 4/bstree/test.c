...
int main() {
  ...
  // Elimino algun elemento
  arbol = bstree_eliminar(arbol, "santa fe", comparar_cadena, destruir_cadena); //, copiar_cadena
  puts("Despues de eliminar...");
  printf("Recorrido inorden: ");
  bstree_recorrer(arbol, BTREE_RECORRIDO_IN, imprimir_cadena, NULL);
  puts("");
  puts("4 elemento: ");
  imprimir_cadena(bstree_k_esimo_menor(arbol, 4), NULL);
  puts("");
  printf("Es un ABB?: %d\n", bstree_validar(arbol, comparar_cadena, imprimir_cadena));
  printf("Es un ABB? 2:  %d\n", bst_validar_2(arbol, comparar_cadena));
