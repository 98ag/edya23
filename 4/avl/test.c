...
int main() {
  ...
  printf("Numero a eliminar: %d.\n", *(numeros + 6));
  avl_eliminar(arbol2, numeros + 6);
  printf("Se elimino %d. Recorrido inorden: ", numeros[6]);
  avl_recorrer(arbol2, AVL_RECORRIDO_IN, imprimir_puntero_entero, NULL);
  puts("");
  if (avl_validar(arbol2) == 1)
    puts("Quedo AVL, funciona bien");
  else
    puts("Pifiaste");
  ...
  /*
  int otrosNumeros[] = {-50, -4, 5, 14, 18, 27, 56};
