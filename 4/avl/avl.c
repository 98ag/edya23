...
static AVL_Nodo *avl_nodo_insertar(AVL_Nodo *raiz, void *dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    if (avl_nodo_factor_balance(raiz) == -2) {
      // casos 1 o 2
      if (avl_nodo_factor_balance(raiz->izq) == 1) // caso 2 (hijo izq, subarbol der: rotacion doble izq + der)
        raiz->izq = avl_nodo_rotacion_simple_izq(raiz->izq);
      raiz = avl_nodo_rotacion_simple_der(raiz); // caso 1 (hijo izq, subarbol izq: rotacion simple der)
    }
    raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
    return raiz;
    ...
    if (avl_nodo_factor_balance(raiz) == 2) {
      // casos 3 o 4
      if (avl_nodo_factor_balance(raiz->der) == -1) // caso 3 (hijo der, subarbol izq: rotacion doble der + izq)
        raiz->der = avl_nodo_rotacion_simple_der(raiz->der);
      raiz = avl_nodo_rotacion_simple_izq(raiz); // caso 4 (hijo der, subarbol der: rotacion simple izq)
    }
    raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
    return raiz;
...
static AVL_Nodo *avl_padre_sucesor_inorder(AVL_Nodo *raiz) {
  printf("Busco sucesor inorder...\n");
  if (raiz->izq == NULL) {
    printf("Resulta ser %d...\n", *(int *)raiz->dato);
    return raiz;
  }
  while (raiz->izq->izq != NULL)
    raiz = raiz->izq;
  printf("Resulta ser %d...\n", *(int *)raiz->dato);
  return raiz;
}

static AVL_Nodo *avl_eliminar_nodo(AVL_Nodo *raiz, void *dato,
                                   FuncionComparadora comp, FuncionDestructora destroy) {

  // printf("Entre al dato %d\n", *(int *)raiz->dato);
  if (raiz == NULL)
    return raiz;

  int resultadoComparacion = comp(raiz->dato, dato);
  ...
  if (resultadoComparacion > 0)
    ...
    // printf("Aca encontre el 18...\n");
    //  printf("%p\n", raiz->izq);
    if (raiz->izq == NULL && raiz->der == NULL) {
      // printf("Entro al caso izq null y der null ...\n");
      destroy(raiz->dato);
      free(raiz);
      return NULL;
    }
      ...
      // printf("Entro al caso izq no null y der no null ...\n");
      AVL_Nodo *padreSucesor = avl_padre_sucesor_inorder(raiz->der);
      AVL_Nodo *sucesor = padreSucesor->izq == NULL ? padreSucesor : padreSucesor->izq;

      // printf("Padre sucesor es %d ...\n", *(int *)padreSucesor->dato);
      // printf("Sucesor es %d ...\n", *(int *)sucesor->dato);

      padreSucesor->izq = (sucesor->der == NULL) ? NULL : sucesor->der;
      sucesor->izq = raiz->izq;
      sucesor->der = (raiz->der == sucesor) ? sucesor->der : raiz->der;

      destroy(raiz->dato);
      free(raiz);

      sucesor->altura = 1 + avl_nodo_max_altura_hijos(sucesor);
      printf("Nueva altura de %d: %d\n", *(int *)sucesor->dato, sucesor->altura);

      return sucesor;
    }

    else {
      AVL_Nodo *aux = raiz->izq == NULL ? raiz->der : raiz->izq;
      destroy(raiz->dato);
      free(raiz);
      aux->altura = 1 + avl_nodo_max_altura_hijos(aux);
      printf("Nueva altura de %d: %d\n", *(int *)aux->dato, aux->altura);
      return aux;
    }
  }

  raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
  printf("Nueva altura de %d: %d\n", *(int *)raiz->dato, raiz->altura);
  // int balancefactor = avl_nodo_factor_balance(raiz);
